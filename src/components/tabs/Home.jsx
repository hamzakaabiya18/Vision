...
const SEED_FEED = [
  { _id:'s1', user:{ username:'marcus_runs', fullName:'Marcus Chen', avatarUrl:'https://images.unsplash.com/photo-1499996860823-5214fcc65f8f?w=80&q=80', verified:true }, sportType:'Run', title:'Morning Summit Chase', distanceKm:14.2, durationMinutes:68, avgPace:'4:50', elevationGainM:312, location:'Carmel Valley, CA', imageUrl:'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=600&q=80', likes:['a','b','c','d'], comments:['x','y'], createdAt: new Date(Date.now()-7200000).toISOString() },
  { _id:'s2', user:{ username:'sara_cycles', fullName:'Sara Valeri', avatarUrl:'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=80&q=80', verified:true }, sportType:'Ride', title:'Pacific Coast Highway Blast', distanceKm:48.6, durationMinutes:112, avgSpeed:26.4, elevationGainM:180, location:'Malibu, CA', imageUrl:'https://thumbs.dreamstime.com/b/young-girl-posing-standing-bike-road-sunset-time-bicycle-77162790.jpg', likes:['a','b','c','d','e','f'], comments:['x','y','z'], createdAt: new Date(Date.now()-18000000).toISOString() },
  { _id:'s3', user:{ username:'leo_trails', fullName:'Leo Brooks', avatarUrl:'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=80&q=80', verified:false }, sportType:'Hike', title:'Timberline Ascent', distanceKm:8.7, durationMinutes:202, elevationGainM:890, location:'Mount Hood, OR', imageUrl:'https://images.unsplash.com/photo-1551632811-561732d1e306?w=600&q=80', likes:['a','b','c','d','e','f','g','h'], comments:['x','y','z','w'], createdAt: new Date(Date.now()-86400000).toISOString() },
  { _id:'s4', user:{ username:'nadia_tri', fullName:'Nadia Kowalski', avatarUrl:'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=80&q=80', verified:true }, sportType:'Swim', title:'Open Water 3K — Lake Geneva', distanceKm:3, durationMinutes:54, avgPace:'18:00', elevationGainM:0, location:'Lake Geneva, CH', imageUrl:'https://images.unsplash.com/photo-1530549387789-4c1017266635?w=600&q=80', likes:['a','b','c'], comments:['x'], createdAt: new Date(Date.now()-172800000).toISOString() },
]
...
function FeedCard({ item, currentUserId, onLike, onComment, onShare }) {
  ...
  return (
    <article style={{ background:'#fff', borderRadius:20, overflow:'hidden', boxShadow:'0 2px 8px rgba(0,128,128,0.08)', border:'1px solid #e8f4f4' }}>
      ...
      <div style={{ display:'flex', padding:'10px 16px 14px', gap:18, alignItems:'center' }}>
        <button onClick={() => onLike(item._id)} style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'none', cursor:'pointer', padding:'4px 0' }}>
...
function CommentSheet({ item, onClose, showToast }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.4)', zIndex:500, display:'flex', alignItems:'flex-end' }} onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={{ width:'100%', maxWidth:430, margin:'0 auto', background:'#fff', borderRadius:'20px 20px 0 0', padding:'20px 16px 36px', maxHeight:'70vh', display:'flex', flexDirection:'column' }}>
        <div style={{ width:36, height:4, borderRadius:2, background:'#ddd', margin:'0 auto 18px' }} />
        <p style={{ fontSize:16, fontWeight:700, color:'#1a1a2e', marginBottom:14 }}>Comments ({item.comments?.length || 0})</p>
        <div style={{ flex:1, overflowY:'auto', marginBottom:14 }}>
          {(!item.comments || item.comments.length === 0) && <p style={{ fontSize:14, color:'#9aaab8', textAlign:'center', padding:'24px 0' }}>No comments yet — be the first!</p>}
        </div>
        <div style={{ display:'flex', gap:10 }}>
          <input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter'){ showToast('Comment posted'); setText(''); onClose() }}} placeholder="Add a comment…" style={{ flex:1, height:44, borderRadius:22, border:'1.5px solid #e8f4f4', padding:'0 16px', fontSize:14, color:'#1a1a2e', outline:'none', fontFamily:'inherit' }} />
          <button onClick={() => { showToast('Comment posted'); setText(''); onClose() }} style={{ height:44, paddingInline:18, borderRadius:22, background:'#008080', color:'#fff', border:'none', fontWeight:600, fontSize:14, cursor:'pointer' }}>Post</button>
        </div>
      </div>
    </div>
  )
...
export default function Home({ user, onNav, showToast }) {
  ...
  useEffect(() => {
    ...
    setLoadingFeed(true)
    fetch(`${API}/activities/feed?limit=10`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleLike = useCallback((id) => {
    setLikedSet(prev => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next })
    setFeed(f => f.map(p => p._id===id ? { ...p, likes: likedSet.has(id) ? (p.likes||[]).filter(x=>x!==currentUserId) : [...(p.likes||[]), currentUserId] } : p))
  }, [likedSet, currentUserId])

  const handleShare = useCallback((item) => {
    if (navigator.share) navigator.share({ title: item.title, text: `Check out ${item.user.fullName}'s ${item.sportType} on VISION` })
  ...
  return (
    <div style={{ background:'#F0FAFA', minHeight:'100%', paddingBottom:32 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'52px 16px 14px', background:'#F0FAFA' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <VisionMark size={30} />
          <div>
            <p style={{ fontSize:11, color:'#9aaab8', fontWeight:500, letterSpacing:'0.06em', textTransform:'uppercase' }}>Good morning</p>
      ...
      <div style={{ margin:'0 16px', borderRadius:20, background:'linear-gradient(145deg,#005f5f 0%,#007a7a 55%,#009a6a 100%)', padding:'20px 20px 18px', boxShadow:'0 6px 28px rgba(0,128,128,0.28)' }}>
        <div style={{ display:'flex', alignItems:'flex-start', marginBottom:4 }}>
          {[{ val:'124.6', label:'DISTANCE', unit:'km' },{ val:'8.4k', label:'CALORIES', unit:'kcal' },{ val:'142', label:'AVG HR', unit:'BPM' }].map(({ val, label, unit }, i) => (
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, margin:'10px 0 12px' }}>
          ...
          <span style={{ fontSize:12, color:'#00E676', fontWeight:600 }}>+12% vs last month</span>
        </div>
        <div>
      ...
      <div style={{ padding:'8px 16px 0' }}>
        ...
        {loadingFeed && <div style={{ textAlign:'center', padding:'24px 0' }}><div style={{ width:32, height:32, borderRadius:'50%', border:'3px solid #e8f4f4', borderTopColor:'#008080', animation:'spin .8s linear infinite', margin:'0 auto' }} /></div>}
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {feed.map(post => <FeedCard key={post._id} item={post} currentUserId={currentUserId} onLike={handleLike} onComment={setCommentItem} onShare={handleShare} />)}
        </div>
      </div>

      {commentItem && <CommentSheet item={commentItem} onClose={() => setCommentItem(null)} showToast={showToast} />}
    </div>
  )
}
